...
public class ProjectorSim : MonoBehaviour
{
    ...
    Cookie[] cookies; // Debug only , set to private when official
    ...
    public void Awake ()
    {
        ...
        if (EditorApplication.isPlaying)
        {
            ...
            cookies[0] = new Cookie(new CookieData(shift_v, shift_h, keystoneH, keystoneV, throwRatio, aspectRatio), cookieSizeInt, lights[0], lights[1], lights[2], images[0], colour);
        ...
        else // EDITOR EDIT MODE - only calculate the preview cookie
        {
            ...
            cookies[0] = new Cookie(new CookieData(shift_v, shift_h, keystoneH, keystoneV, throwRatio, aspectRatio), cookieSizeInt, lights[0], lights[1], lights[2], images[previewImageIndex], colour);
        }
#endif
        AssignLightCookies();                
    }
    ...
    public void OnEnable() //Debug public only, set to private when official
    {
...
// allow slideshow in editor play mode
    ...
    public void OnDisable() // Public so other scripts can access. Might turn off when official.
    {
        foreach (Light l in lights)
        {
            l.enabled = false;
    ...
    /// </summary>
    public void AssignLightCookies() // Need to be public so it can be accessed by Awake() from other script Debug public only, set to private once official
    {
